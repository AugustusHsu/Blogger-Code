...
def generate_and_save_images(pred_img, epoch, SaveName):
    # Create folder
    fig_path = os.path.join(FLAGS.PLOT_PATH)
    if not os.path.exists(fig_path):
        os.mkdir(fig_path)
    fig_path = os.path.join(fig_path, SaveName)
    if not os.path.exists(fig_path):
        os.mkdir(fig_path)
...
def plot_GIF(SaveName):
    anim_file = '{}.gif'.format(SaveName)
    if not os.path.exists(FLAGS.IMG_PATH):
        os.mkdir(FLAGS.IMG_PATH)
    images = []
    img_path = os.path.join(FLAGS.PLOT_PATH, SaveName)
...
def plot_line(df, col_name, SaveName, figname):
    # Create folder
    ...
    path = os.path.join(fig_path, SaveName + '_' + figname + '.png')
    plt.savefig(path)
    plt.close('all')
